# BODACC.io : lista de URLs

> 🌍 [Français](../fr/URLLIST.md) · [Español](../es/URLLIST.md) · [Deutsch](../de/URLLIST.md) · [Italiano](../it/URLLIST.md) · [Português](URLLIST.md) · [Nederlands](../nl/URLLIST.md) · [Polski](../pl/URLLIST.md) · [Русский](../ru/URLLIST.md) · [Türkçe](../tr/URLLIST.md) · [Українська](../uk/URLLIST.md) · [Bahasa Indonesia](../id/URLLIST.md) · [Tiếng Việt](../vi/URLLIST.md) · [日本語](../ja/URLLIST.md) · [한국어](../ko/URLLIST.md) · [中文](../zh/URLLIST.md) · [العربية](../ar/URLLIST.md) · [עברית](../he/URLLIST.md) · [हिन्दी](../hi/URLLIST.md) · [فارسی](../fa/URLLIST.md) · [English](../en/URLLIST.md)
Pontos de entrada do site : mantidos para crawlers, bots e agentes.
Todos os URLs abaixo retornam HTTP 200 e são publicamente acessíveis.

## Site (multilíngue : 20 idiomas, /pt padrão)

- Início: https://bodacc.io/pt
- Anúncios: https://bodacc.io/pt/annonces
- Empresas: https://bodacc.io/pt/entreprises
- Pessoas: https://bodacc.io/pt/personnes
- Atores (administradores judiciais): https://bodacc.io/pt/acteurs
- Preços: https://bodacc.io/pt/tarifs
- Tecnologia / dados abertos: https://bodacc.io/pt/technologie
- Hub de regiões: https://bodacc.io/pt/regions
- Cidades: https://bodacc.io/pt/villes

## Descoberta e feeds

- RSS feed (daily recaps + latest notices): https://bodacc.io/rss.xml
- Sitemap index: https://bodacc.io/sitemap.xml
- robots.txt: https://bodacc.io/robots.txt
- llms.txt (for AI agents): https://bodacc.io/llms.txt
- ads.txt: https://bodacc.io/ads.txt
- OpenAPI spec: https://bodacc.io/api/bodacc/openapi.json

## API (pública, sem chave)

- Base: https://bodacc.io/api/bodacc
- Notices search: https://bodacc.io/api/bodacc/annonces?limit=20
- Notice detail: https://bodacc.io/api/bodacc/annonces/A202601511039
- Companies search: https://bodacc.io/api/bodacc/entreprises?q=paris&limit=20
- Company by SIREN: https://bodacc.io/api/bodacc/entreprises/514494491
- Company by slug: https://bodacc.io/api/bodacc/entreprises/by-slug/042285445-copr-l-edelweiss
- People: https://bodacc.io/api/bodacc/personnes/397362
- Live counters: https://bodacc.io/api/bodacc/stats/counts
- Daily stats: https://bodacc.io/api/bodacc/stats/daily
- NAF reference: https://bodacc.io/api/bodacc/naf

## Exemplos (páginas de entidades)

- Example company page (pt): https://bodacc.io/pt/entreprises/514494491
- Example notice page (pt): https://bodacc.io/pt/annonces/A202601511039

